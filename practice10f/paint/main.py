...
def draw_panel(screen, current_color, current_tool, brush_size):
    ...
    for i, tool in enumerate(TOOLS):
        x = 480 + i * 75
        button_color = (100, 200, 100) if tool == current_tool else (180, 180, 180)

        pygame.draw.rect(screen, button_color, (x, 8, 70, 35), border_radius=5)
        pygame.draw.rect(screen, (0, 0, 0), (x, 8, 70, 35), 1, border_radius=5)

        text = font.render(tool_labels[tool], True, (0, 0, 0))
        screen.blit(text, (x + 8, 17))
...
def main():
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if my < PANEL_H:
                    ...
                    for i, tool in enumerate(TOOLS):
                        x = 480 + i * 75
                        tool_rect = pygame.Rect(x, 8, 70, 35)
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                if drawing and start_pos:
                    ...
                    if current_tool == "rect":
                        draw_rectangle(canvas, start_pos, end_pos, current_color, brush_size)
        ...
        if drawing and start_pos and current_tool in ("rect", "circle"):
            ...
            if current_tool == "rect":
                draw_rectangle(preview, start_pos, end_pos, current_color, brush_size)

            elif current_tool == "circle":
                draw_circle(preview, start_pos, end_pos, current_color, brush_size)
